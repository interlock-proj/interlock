...
class SampleTrackedCommand(IdempotencyTrackedCommand):
    """Sample command with idempotency tracking."""
...
async def test_in_memory_backend_stores_and_retrieves(command):
    ...
    # Initially, command should not be processed
    assert await backend.has_processed_command(command) is False

    # Store the command
    await backend.store_processed_command(command)

    # Now it should be marked as processed
    assert await backend.has_processed_command(command) is True
...
async def test_in_memory_backend_different_keys_independent():
    ...
    command1 = SampleTrackedCommand(aggregate_id=ULID(), idempotency_key="key-1")
    command2 = SampleTrackedCommand(aggregate_id=ULID(), idempotency_key="key-2")

    await backend.store_processed_command(command1)

    # Only command1 should be marked as processed
    assert await backend.has_processed_command(command1) is True
    assert await backend.has_processed_command(command2) is False


@pytest.mark.asyncio
async def test_in_memory_backend_same_key_different_command():
    """Test that same idempotency key on different commands is recognized."""
    backend = InMemoryIdempotencyStorageBackend()

    command1 = SampleTrackedCommand(aggregate_id=ULID(), idempotency_key="same-key")
    command2 = SampleTrackedCommand(
        aggregate_id=ULID(), idempotency_key="same-key"
    )  # Different aggregate, same key

    await backend.store_processed_command(command1)

    # Both commands have same key, so both should be marked as processed
    assert await backend.has_processed_command(command1) is True
    assert await backend.has_processed_command(command2) is True


@pytest.mark.asyncio
async def test_in_memory_backend_multiple_stores_idempotent():
    """Test that storing the same command multiple times is idempotent."""
    backend = InMemoryIdempotencyStorageBackend()
    command = SampleTrackedCommand(aggregate_id=ULID(), idempotency_key="test-key")
