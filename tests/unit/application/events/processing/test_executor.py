...
from interlock.context import get_context, clear_context
...
def executor(processor):
    ...
    return EventProcessorExecutor(
        processor, Never(), NoCatchup(), batch_size=3
    )
...
def test_executor_init_custom_batch_size(processor):
    ...
    executor = EventProcessorExecutor(
        processor, Never(), NoCatchup(), batch_size=50
    )
...
async def test_process_batch_routes_events(executor, processor, transport):
    ...
    sub = await publish_and_subscribe(transport, [
...
async def test_process_batch_calculates_lag(transport, processor, now):
    ...
    executor = EventProcessorExecutor(
        processor, Never(), NoCatchup(), batch_size=2
    )
    ...
    assert timedelta(minutes=9, seconds=50) <= avg_age <= timedelta(
        minutes=10, seconds=10
    )


@pytest.mark.asyncio
async def test_process_batch_respects_skip_window(
    executor, processor, transport, now
):
...
@pytest.mark.asyncio
async def test_process_batch_returns_zero_when_all_skipped(
    transport, processor, now
):
...
async def test_process_batch_restores_context(transport):
    ...
    executor = EventProcessorExecutor(
        ContextCaptor(), Never(), NoCatchup(), batch_size=2
    )
    correlation_id = ULID()
...
@pytest.mark.asyncio
async def test_process_batch_handles_missing_correlation_id(
    executor, processor, transport
):
...
async def test_batch_and_catchup_no_trigger(transport):
    ...
    executor = EventProcessorExecutor(
        processor, Never(), strategy, batch_size=2
    )
...
async def test_batch_and_catchup_triggers(transport):
    ...
    executor = EventProcessorExecutor(
        processor, AfterNEvents(1), strategy, batch_size=1
    )

    sub = await publish_and_subscribe(transport, [
        event(AccountOpened(owner=f"User{i}")) for i in range(3)
    ])
...
async def test_batch_and_catchup_with_skip_window(transport, now):
    ...
    processor = AccountStatisticsProcessor()
    executor = EventProcessorExecutor(
        processor, Never(), MockCatchupStrategy(), batch_size=2
    )
    old = now - timedelta(minutes=10)
    ...
    skip_before = now - timedelta(minutes=5)
    result = await executor.process_batch_and_check_catchup(
        sub, CatchupResult(skip_before)
    )
...
async def test_run_executes_initial_catchup(processor, transport):
    ...
    strategy = MockCatchupStrategy()
    executor = EventProcessorExecutor(
        processor, Never(), strategy, batch_size=1
    )
    sub = await transport.subscribe("test")
...
async def test_run_processes_multiple_batches(transport):
    ...
    sub = await publish_and_subscribe(transport, [
        event(AccountOpened(owner=f"User{i}")) for i in range(4)
    ])
...
async def test_executor_propagates_exceptions(transport):
    ...
    executor = EventProcessorExecutor(
        FailingProcessor(), Never(), NoCatchup(), batch_size=1
    )
    sub = await publish_and_subscribe(transport, [
        event(AccountOpened(owner="Alice")),
    ])
...
async def test_executor_calculates_lag_accurately(transport, processor, now):
    ...
    assert timedelta(minutes=4, seconds=50) <= avg_age <= timedelta(
