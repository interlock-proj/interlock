"""Integration tests for MongoDBEventStore using real MongoDB via testcontainers."""
...
async def event_store(connection_manager):
    ...
    store = MongoDBEventStore(connection_manager)
    await store.initialize_schema()
    yield store
    # Cleanup: Delete all data after each test
    await connection_manager.database["events"].delete_many({})
    await connection_manager.database["aggregates"].delete_many({})
...
async def test_save_and_load_single_event(event_store):
    ...
    assert loaded_events[0].data.customer_id == "CUST123"  # type: ignore[attr-defined]
    assert loaded_events[0].data.total == 99.99  # type: ignore[attr-defined]
...
async def test_multiple_aggregates_isolation(event_store):
    ...
    assert agg1_events[0].data.customer_id == "CUST1"  # type: ignore[attr-defined]
    assert agg2_events[0].data.customer_id == "CUST2"  # type: ignore[attr-defined]


@pytest.mark.asyncio
async def test_save_empty_event_list(event_store):
    """Test saving empty event list does nothing."""
    await event_store.save_events([], expected_version=0)
    # Should return early without any DB operations - no error


@pytest.mark.asyncio
async def test_event_with_correlation_and_causation_ids(event_store):
    """Test events with correlation and causation IDs are preserved."""
    aggregate_id = ULID()
    correlation_id = ULID()
    causation_id = ULID()
    ...
    assert len(loaded_events) == 1
    assert loaded_events[0].correlation_id == correlation_id
    assert loaded_events[0].causation_id == causation_id
